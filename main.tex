...
Throughout this paper, $\Bbbk$ is a field, and $\bfP=(\bfP,\le)$ is a poset.
We regard $\bfP$ as a small (thin) category: there is a unique morphism $x\to y$ exactly when $x\le y$. This convention lets us treat $\bfP$-modules as functors $\bfP\to\vect$ and, equivalently, as left modules over the $\Bbbk$-linearization $\Bbbk\bfP$ (same objects, order relations as basis arrows, composition determined by transitivity). We assume $\bfP$ is finite except in Sect.\ \ref{sec:intlv-dist}.

The category of finite-dimensional $\Bbbk$-vector spaces is denoted by $\vect$. We write $\vect^\bfP=\Fun(\bfP,\vect)$ for the category of $\bfP$-modules. If $\bfP$ is finite, then $\vect^\bfP$ is a $\Bbbk$-linear, abelian, Krull--Schmidt category, and for each pair of $\bfP$-modules $M,N$ the Hom-space $\vect^\bfP(M,N)$ is finite-dimensional.

We write $\prj\bfP$ for the full subcategory of projective objects in $\vect^\bfP$.
Given $x\in\bfP$, the \emph{representable} at $x$ is $\Yp{x}:=\bfP(x,-)\in\vect^\bfP$. 
It assigns to $y\in\bfP$ the vector space $\Yp{x}(y)=\Bbbk$ if $x\le y$ and $0$ otherwise, and on a relation $y\le z$ acts by the identity on $\Bbbk$ whenever $x\le y$ (and by $0$ otherwise). 
Representables encode evaluation: by the Yoneda lemma there is a natural isomorphism
\[
\Hom_{\vect^\bfP}(\Yp{x},M)\cong M(x)
\]
for every $\bfP$-module $M$. 
It follows that each $\Yp{x}$ is projective, and these are the indecomposable projectives in $\vect^\bfP$. The following is well known; we omit the proof.

\begin{lemma}
\label{lem:ind-proj}
The family $\{\Yp{x}\mid x\in\bfP\}$ forms a complete set of representatives of isoclasses of indecomposable projective $\bfP$-modules. Hence each $M\in\prj \bfP$ is expressed as
\[
M \cong \bigoplus_{x\in\bfP} \Yp{x}^{\,a_x}
\]
by a unique family $(a_x)_{x\in\bfP}\in\bbN^\bfP$.
\end{lemma}
...
A map $f:\bfP\to\bfQ$ between posets is \emph{monotone} if $x\le y$ in $\bfP$ implies $f(x)\le f(y)$ in $\bfQ$ for all $x,y\in\bfP$. Viewing posets as thin categories, a functor $\bfP\to\bfQ$ is the same thing as a monotone map. Hence a monotone map $f:\bfP\to\bfQ$ induces the restriction functor
\[
...
\begin{proposition}
For any monotone $f:\bfP\to\bfQ$, the functor $f^{*}$ is exact. Moreover, the left and right Kan extensions along $f$ exist and are given objectwise by
\[
(f_{!}M)(q)=\varinjlim_{\{\,x\in\bfP\mid f(x)\le q\,\}} M(x),
\qquad
(f_{*}M)(q)=\varprojlim_{\{\,x\in\bfP\mid q\le f(x)\,\}} M(x),
\]
and there are natural adjunctions $f_{!}\dashv f^{*}\dashv f_{*}$. In particular, $f_{!}$ is right exact, $f_{*}$ is left exact, and $f^{*}$ is exact.
\end{proposition}

We record a general fact that we will use later.
...
\begin{proof}
Let $A$ be projective in $\calC$. Then $\calC(A,-)$ is exact, hence so is
...
We recall Galois connections, which are adjunctions between posets and underlie the identifications among $f_{!}, f^{*}, f_{*}$ used later.
...
If $\calC$ is an object of $\Cat^\frkU$, then $\Cat^\frkU(\mbox{-},\calC):(\Cat^\frkU)^{\mathrm{op}}\to\Cat^\frkU$ is a $2$-functor.
\end{proposition}
...
\begin{corollary}
If $f:\bfP\rightleftarrows \bfQ:g$ is a Galois connection between posets $\bfP,\bfQ$, then $g^{*}\dashv f^{*}$.
$$
\begin{tikzcd}[row sep=3em]
\Nname{E}\vect^\bfP &\Nname{C} \vect^\bfQ 
\arrow[from={E}, to={C}, "g^{*}", ""'{name=Fd}, bend left]
\arrow[from={C}, to={E}, "f^{*}" pos=0.48, ""'{name=Gu}, bend left]
...
\begin{proof}
With $\vect$ chosen $\frkU$-small, Proposition~\ref{prp:Yoneda-2-functor} applied to $\calC:=\vect$ shows that $(\mbox{-})^{*}:=\Cat^\frkU(\mbox{-},\vect):(\Cat^\frkU)^{\mathrm{op}}\to\Cat^\frkU$ is a $2$-functor. It sends an adjoint system $(f,g,\eta,\varepsilon)$ to $(g^{*},f^{*},\eta^{*},\varepsilon^{*})$.
\end{proof}
...
\Nname{E}\vect^\bfP &\Nname{C} \vect^\bfQ 
\arrow[from={E}, to={C}, "g^{*} \cong f_{!}"{name=Fu}, ""'{name=Fd}, bend left=15pt]
\arrow[from={C}, to={E}, "f^{*}\cong g_{*}"{name=Gd}, ""'{name=Gu}, bend left=15pt]
...
\begin{proof}
Since $g^{*}\dashv f^{*}$ and $f_{!}\dashv f^{*}$, we obtain $g^{*}\cong f_{!}$. Similarly, $f^{*}\cong g_{*}$.
\end{proof}
...
\begin{proof}
By Corollary~\ref{cor:3_adj}, $g_{*}\cong f^{*}$, and $f^{*}$ is exact.
\end{proof}
...
\item For $q\in \bfQ$, $g(q)=\max\{\,x\in \bfP\mid f(x)\le q\,\}$. In particular, if $f\circ g=\id_{\bfQ}$, then $g(q)=\max\{\,x\in \bfP\mid f(x)=q\,\}$.
\end{enumerate}
\end{lemma}

\begin{proof}
(1) From $fgf=f$ and $gfg=g$ one deduces the three conditions are equivalent. 
(2) The Galois equivalence $x\le g(q)\iff f(x)\le q$ holds for all $x\in\bfP$ and $q\in\bfQ$, and implies the stated formulas.
\end{proof}

\begin{definition}
A Galois connection $f:\bfP\rightleftarrows \bfQ:g$ is a \emph{Galois insertion} if the equivalent conditions in (1) above hold.
\end{definition}
