...
\[
\bfR \ :=\ \bigl\{(q_1,q_2)\in\bfQ_1\times\bfQ_2 : h_1(q_1)=f_2(q_2)\bigr\},
\]
...
\[
\Gamma_1(q_1)\to \Gamma_1\bigl(i_1 h_1(q_1)\bigr) \cong N\bigl(h_1(q_1)\bigr),
\qquad
\Gamma_2(q_2)\to \Gamma_2\bigl(g_2 f_2(q_2)\bigr) \cong N\bigl(f_2(q_2)\bigr).
\]
...
\Gamma_2(q_2) \ar[d, "\Gamma_2\bigl(q_2\le g_2 f_2(q_2)\bigr)"] \\
\Gamma_1(q_1) \ar[r, "\Gamma_1\bigl(q_1\le i_1 h_1(q_1)\bigr)"'] &
N\bigl(h_1(q_1)\bigr) = N\bigl(f_2(q_2)\bigr).
\end{tikzcd}
\]
...
$g_1^{*}\Gamma_1\cong M$ gives a canonical identification
\[
\Gamma_1(q_1)\ \cong\ M(p).
\]
Moreover, since $q_2=g_2 h_1(q_1)$, the structure map 
\[
\Gamma_2\bigl(q_2\le g_2 f_2(q_2)\bigr)
\]
is an isomorphism.

Thus the pullback square becomes
\[
\begin{tikzcd}[column sep=7.5em]
\Psi(q_1,q_2) \ar[r, dashed] \ar[d, dashed] &
\Gamma_2(q_2) 
   \ar[d, "\Gamma_2\bigl(q_2\le g_2 f_2(q_2)\bigr)", "\cong"'] \\
\Gamma_1(q_1)\cong M(p)
   \ar[r, "\Gamma_1\bigl(q_1\le i_1 h_1(q_1)\bigr)"'] &
N\bigl(h_1(q_1)\bigr),
\end{tikzcd}
\]
and hence $\Psi(q_1,q_2)\ \cong\ M(p)$.
Naturality yields $(\iota_1 g_1)^{*}\Psi\cong M$.
...
$i_2^{*}\Gamma_2\cong O$ gives a canonical identification
\[
\Gamma_2(q_2)\ \cong\ O(p).
\]
Moreover, since $q_1=i_1 f_2(q_2)$, the structure map 
\[
\Gamma_1\bigl(q_1\le i_1 h_1(q_1)\bigr)
\]
is an isomorphism.
...
\Psi(q_1,q_2) \ar[r, dashed] \ar[d, dashed] &
\Gamma_2(q_2)\cong O(p)
   \ar[d, "\Gamma_2\bigl(q_2\le g_2 f_2(q_2)\bigr)"'] \\
\Gamma_1(q_1)
   \ar[r, "\Gamma_1\bigl(q_1\le i_1 h_1(q_1)\bigr)", "\cong"'] &
N\bigl(h_1(q_1)\bigr),
\end{tikzcd}
\]
and hence $\Psi(q_1,q_2)\ \cong\ O(p)$.
Naturality yields $(\iota_2 i_2)^{*}\Psi\cong O$.
...
\[
f_1\pi_1 \dashv \iota_1 g_1,
\qquad
h_2\pi_2 \dashv \iota_2 i_2.
\]
Together with the identifications above, this completes the proof.
\end{proof}
